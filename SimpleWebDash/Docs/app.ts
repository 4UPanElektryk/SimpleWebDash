...
let SubscribedHTTP: Array<DataAssociacion> = [];
let SubscribedIP: Array<DataAssociacion> = [];
...
async function GetTempsOfServer(host: string): Promise<ServerDataResponse<TemperatureEndpointResponseData>> {
	try {
		const response = await fetch(`/api/tempstats?ip=${host}&t=${timespan.value}`, { signal: AbortSignal.timeout(1500) });
