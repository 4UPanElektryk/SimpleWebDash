...
public class CombinedMemoryE : DataEndpoint
{
	...
	public override HttpResponse ReturnData(HttpRequest request)
	{
		...
		ServerDataResponse<CombinedMemoryResponse> response1 = new()
		{
			Type = responseType,
			Message = message,
	...
	{
		Dictionary<string, Task<MemoryMonitorData[]>> allData = new();
		...
		Dictionary<string, MemoryResponse> allDataResult = new();
		foreach (var node in allData)
		{
			List<long> timestamps = new();
			List<ulong> totalkbs = new();
			List<ulong> usedkbs = new();
